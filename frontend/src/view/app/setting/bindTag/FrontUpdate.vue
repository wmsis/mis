        ...
        <Form ref="formRef"
            ...
            <FormItem prop="historian_tagId" label="后端标识" style="text-align: left;">
                <Select placeholder="请输入搜索TAG关键字" filterable multiple v-if="!tagLoading"
                    ...
                    <Option v-for="(item, index) in tagOptionAll" :value="item.id" v-if="item.tag_name != 'fake_historian_tag'"
    ...
    export default {
        ...
        methods: {
            visibleChange: function (visible) {
                if (visible) {
                    let SELF = this;
                    SELF.formData.front_tagId = SELF.data.front_tag_id;
                    SELF.formData.func = SELF.data.func;
                    SELF.formData.description = SELF.data.description;
                    SELF.formData.name = SELF.data.name;
                    SELF.formData.measure = SELF.data.name;
                    SELF.isFunc = SELF.data && SELF.data.func ? true : false;
                    SELF.datalist = SELF.data && SELF.data.color_list ? JSON.parse(SELF.data.color_list) : [];
                    ...
                    let tagstr = SELF.data.historian_tag_id;
                    let arr;
                    if(tagstr.indexOf(',') !== -1){
                        arr = tagstr.split(',');
                    ...
                    SELF.formData.historian_tagId = arr;
                    SELF.tags();
                } else {
            ...
            tags(){
                ...
                SELF.ajax({
                    ...
                    success: function (data) {
                        SELF.tagList = data;
                        SELF.tagOptionAll = SELF.initTagAll();
            ...
            handleSubmit: function (name) {
                ...
                vm.$refs[name].validate((valid) => {
                    ...
                    if (vm.formData.historian_tagId) {
                        let tagarr= vm.formData.historian_tagId;
                        let tagstr = tagarr.join(',');
                        rqData.historian_tag_id = tagstr;
                    }
                    ...
                    vm.ajax({
                        method: 'POST',
                        url: vm.$request.Boiler.FrontUpdate.replace('{id}', this.data.id),
                        params: rqData,
                        success: function () {
