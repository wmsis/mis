    ...
    <div>
        <BaseTable
            ...
            <template slot="info" slot-scope="{ row }">
                <div class="wx-info">
                    <img :src="row.member.wechats.length > 0 ? row.member.wechats[0].headimgurl : defalutImg"  width="50" height="50"/>
                    <span>{{row.member.username ? row.member.username : row.member.wechats.length > 0 ? row.member.wechats[0].nickname : '无名'}}</span>
                </div>
            </template>
