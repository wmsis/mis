    ...
    <div>
        ...
        <base-table ref="tableRef"
            ...
            <template slot="option" slot-scope="{ row }">
                <div>
                    <Button  type="text" :disabled="row.id==1 ? true : false" size="small" class="btn-my" @click.stop="update(row)" v-if="isButtonExist('role-update')">修改</Button>
                    <Button  type="text" :disabled="row.id==1 ? true : false" size="small" class="btn-my" @click.stop="remove(row)" v-if="isButtonExist('role-delete')">删除</Button>
                    <Button  type="text" :disabled="row.id==1 ? true : false" size="small" class="btn-my" @click.stop="assign(row)" v-if="isButtonExist('role-assign-permission')">分配权限</Button>
                </div>
            </template>
        </base-table>
    </div>
