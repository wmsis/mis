    ...
    export default {
        ...
        methods: {
            visibleChange: function(visible) {
                if (visible) {
                    this.getUserOrgnization();
                } else {
                    this.shows.orgnization = false;
                }
            },
            getUserOrgnization() {
                let vm = this;
                vm.loading = true;
                let url = vm.$request.users.orgnizationList.replace("{id}", this.userId);
                vm.ajax({
                    url: url,
                    success: function (data) {
                        vm.userOrgnizations = data.myOrgnizations;
                        vm.setTreeData()
                    }
                });
            },
            ...
            selectIds(lists){
                for(let i=0; i<lists.length; i++){
                    for(let item of this.userOrgnizations){
                        if(item.level <= 3){
                            lists[i].expand = true;
                        }
            ...
            handleSelect: function() {
                ...
                let url = vm.$request.users.assignOrgnizations.replace("{id}", this.userId);
                vm.ajax({
                    url: url,
                    method: 'POST',
                    data: {
