<template>
    <Modal
        ...
        <Form ref="formRef"
              ...
              :label-width="100"
              label-position="left">
            <div class="container-wechat-menu-add">
                <Row>
                    <Col span="24" >
                        <FormItem prop="parent_path" label="级别">
                            <Select v-model="formData.parent_path" class="sel" size="large" placeholder="请选择级别">
                                <Option v-for="item in topMenus" :value="item.path" :key="item.path">{{ item.name }}</Option>
                            </Select>
                        </FormItem>
                    </Col>
                </Row>
        ...
        methods: {
            ...
            handleSubmit: function (name) {
                ...
                vm.$refs[name].validate((valid) => {
                    ...
                    vm.formData.is_root = vm.formData.parent_path == 'top';
                    vm.ajax({
                        method: 'POST',
                        url: vm.$request.wechatMenu.insert,
                        data: {
                            name: vm.formData.name,
                            parent_path: vm.formData.parent_path == 'top' ? '' : vm.formData.parent_path,
                            sort: vm.formData.sort,
                            type: vm.formData.type,
                            keyword: vm.formData.keyword,
                            appid: vm.formData.appid,
                            pagepath: vm.formData.pagepath,
                            url: vm.formData.url,
                            is_open: vm.formData.is_open,
                            is_root: vm.formData.is_root
                        },
                        success: function () {
