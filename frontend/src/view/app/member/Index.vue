    ...
    <div>
        ...
        <base-table ref="tableRef"
            ...
            <template slot="option" slot-scope="{ row }">
                <div v-if="isButtonExist('member-push')">
                    <Button type="text" size="small" class="btn-my"  @click.stop="pushSetting(row)" v-if="row.wechats && row.wechats.length > 0 && row.wechats[0].subscribe">推送设置</Button>
                </div>
            </template>
...
<script>
    ...
    export default {
        ...
        mounted() {
          membUtils.checkTags(this).then((res)=>{
            this.allTags=res;
          });
        },
        methods: {
