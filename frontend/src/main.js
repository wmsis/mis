...
Vue.prototype.isButtonExist = function (name) {
    let flag = false;
    if (store.state.userInfo.id != 2) {
        for (let key in store.state.privileges) {
            let item = store.state.privileges[key];
            if (item.type != 'menu' && item.type != 'root' && item.api_name == name) {
                flag = true;
                break;
            }
        }
