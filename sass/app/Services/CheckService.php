...
class CheckService{
    public function userClassPoint($date)
    {
        ...
        foreach ($date_users_schdule as $key => $user_schdule) {
            $start = $date . ' ' . $user_schdule->start . ':00';
            $end = $date . ' ' . $user_schdule->end . ':00';
            $orgnization = Orgnization::find($user_schdule->orgnization_id);
            if($orgnization){
                $rangedata = $this->getRangeElectricity($orgnization, $start, $end);
                $powerdata = $this->getRangePower($rangedata);
                $final[$user_schdule->user_id] = $powerdata;
            }
        }
    ...
    private function getRangeElectricity($orgnization, $start, $end){
        ...
        $electricity_obj = (new Electricity())->setTable($electricity_table);
