...
class NariDataCmd extends Command
{
     ...
     * @var string
     */
    protected $signature = 'collect:naridata';
     ...
     * @var string
     */
    protected $description = 'collect naridata';
            ...
            foreach ($factories as $k2 => $factory) {
                if($factory->code){
                    ...
                    if($cfg && $map){
                        ...
                        $nari_data_table = 'nari_' . $factory->code; //本地存储数据库表名称
                        $params = array(
                            'tenement_conn' => $tenement_conn,
                            'local_data_table' => $local_data_table,
                            'nari_data_table' => $nari_data_table,
                            'map' => $map->toArray(),
                            'cfgdb' => $cfg->toArray()
                        );

                        dispatch(new NariDataJob($params));
                    }
                }
            }
        }
