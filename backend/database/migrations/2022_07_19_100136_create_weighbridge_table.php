...
class CreateWeighbridgeTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('weighbridge_yongqiang2', function (Blueprint $table) {
            ...
            $table->string('firstdatetime', 30)->nullable()->comment('第一次称重时间');
            $table->string('seconddatetime', 30)->nullable()->comment('第二次称重时间');
            $table->string('grossdatetime', 30)->nullable()->comment('毛重时间');
            $table->string('taredatetime', 30)->nullable()->comment('皮重时间');
            ...
            $table->tinyInteger('datastatus')->nullable()->comment('状态：9为删除，1为正常');
